...
export function RecurringExpenseModal({
  ...
  isOpen: boolean
  existing: RecurringExpenseDetail | null
  ...
  const recurrenceChanged = existing
    ? (() => {
        const origDate = existing.next_payment_date?.split('T')[0] ?? ''
        const origPreset = existing.paid_every ?? ''
        ...
        if (form.next_payment_date !== origDate) return true
        if (form.frequency_mode === 'preset') return form.paid_every !== origPreset
        return (
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setForm({
        expense_name: existing.expense_name,
        ...
        paid_every: existing.paid_every ?? '',
        ...
        next_payment_date: existing.next_payment_date?.split('T')[0] ?? '',
        frequency_mode:
  ...
  const addSplit = () => {
    ...
    if (!next) return
    setSplits([...splits, { family_id: next.id, split_name: form.expense_name || next.name, amount: '' }])
  }
    ...
    if (isSimple) {
      ...
      if (existing) {
        return existing.family_splits.map((s) => ({
          family_id: s.family_id,
          split_name: s.split_name || form.expense_name,
          amount: s.family_id === targetId ? familyAmt : s.amount,
        })).filter((s) => Number(s.amount) > 0)
      }
      return familyAmt > 0
        ? [{ family_id: targetId, split_name: form.expense_name || contextFamilyName || 'Family', amount: familyAmt }]
        : []
    }
    return splits
      .filter((s) => Number(s.amount) > 0)
  ...
  const familyLabel = contextFamilyName ? `From ${contextFamilyName}` : 'From this family'

  const goNext = () => {
    if (step === 0 && !form.expense_name.trim()) {
      toast.error('Enter an expense name before continuing')
      return
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{existing ? 'Edit recurring expense' : 'Add recurring expense'}</DialogTitle>
          <DialogDescription>
            {isSimple
              ? 'Update this expense for this family and your personal savings.'
              : 'Split your expense across any families you belong to, plus personal savings.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Leave blank (and no personal rows) to take the full total from this family.
                </p>
              </div>
              ...
              <p className={`text-xs ${allocationOk ? 'text-positive' : 'text-destructive'}`}>
                {noExplicitSimpleSplit
                  ? 'Entire amount comes from family savings.'
          ...
          <div className="space-y-3 rounded-xl border border-border p-4 bg-muted/10">
            ...
            {form.frequency_mode === 'preset' ? (
              <Select value={form.paid_every} onValueChange={(v) => setForm({ ...form, paid_every: v })}>
          ...
          <RecurringDocumentFields
            ...
            members={members}
            entryNoun="expense"
            helperText="Optional — e.g. bill, receipt, or subscription invoice."
          />
          ...
          {isSimple && (
            <LetEveryoneEditSection
              ...
              allowedDescription="Allow any family member to update this recurring expense."
        ...
        <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              expense entry. Any pending confirmation for the current period may be replaced.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
