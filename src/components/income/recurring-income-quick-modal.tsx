...
export function RecurringIncomeQuickModal({
  isOpen,
  familyId,
  members,
  onCreate,
  ...
  const { data: categoriesData } = useIncomeCategories(familyId)
  const createCategoryMutation = useCreateIncomeCategory(familyId)
  ...
  const handleSubmit = () => {
    ...
    if (!form.next_receiving_date) {
      toast.error('Please choose when you expect to receive this income next.')
      return
    }
    if (form.frequency_mode === 'preset' && !form.received_every) {
      toast.error('Please select how often this income repeats.')
      return
    }
    ...
    if (isSplitting) {
      ...
      const pa = personal ?? 0
      if (fa <= 0 || pa <= 0) {
        toast.error('When splitting income, both family and personal amounts must be greater than zero.')
        return
      }
      if (fa + pa !== total) {
  ...
  const canSubmit =
    ...
    !!form.next_receiving_date &&
    (form.frequency_mode === 'preset' ? !!form.received_every : true)
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Set up income you receive on a regular schedule for this family.
            To split across multiple families, use <span className="font-medium">Personal → Income</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          <div className="flex items-start gap-3 rounded-xl border border-brand/20 bg-brand/5 p-3">
            ...
            <div>
              <p className="text-sm font-medium">Your income only</p>
              <p className="text-xs text-muted-foreground">
                Recurring income must be added by the person who earns it.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              />
              <p className="text-xs text-muted-foreground">Leave blank to credit the full amount to this family.</p>
            </div>
            <div className="space-y-1">
          ...
          <div className="space-y-3 rounded-xl border border-border p-4 bg-muted/10">
            ...
            {form.frequency_mode === 'preset' ? (
              <div className="space-y-1">
                ...
                <Select
                  value={form.received_every}
                  onValueChange={(v) => setForm({ ...form, received_every: v })}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-1 pt-1">
              ...
              <Input
                type="date"
                value={form.next_receiving_date}
                onChange={(e) => setForm({ ...form, next_receiving_date: e.target.value })}
              />
              <p className="text-xs text-muted-foreground">
                When do you expect the next payment? We use this to schedule your first automatic income entry
            ...
            <div className="space-y-3 rounded-xl border border-border p-4">
              ...
              <div className="flex items-center justify-between gap-3 border-t border-border/50 pt-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    When enabled, the same file is added to each automatic income entry
                    (handy for a recurring payslip or invoice).
                  </p>
                </div>
                <Switch
              ...
              {form.repeat_doc_with_logs && (
                <p className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  This document will be linked to this income and reused on future entries.
                </p>
              )}
            </div>
          )}
          ...
          <LetEveryoneEditSection
            ...
            allowedDescription="When on, any family member can update this recurring income from Manage recurring income."
